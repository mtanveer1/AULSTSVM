...
%             c2=c;
            ...
            for  ei = 1:length(epsv)
                   ...
                   %% testing and training
                                [accuracy(i),time] = Lin_AULSTSVM(Data,B_t,U,c,c2,a,e);
...
%              [accuracy_with_zero,yt,yp,time] = gunnsvc(Data(:,1:no_col-1),Data(:,no_col),B_t(:,1:no_col-1),B_t(:,no_col),c);
%              fprintf(file2,'file %s c1 %8.6g avgerror %8.6g\n',file,c,p1,accuracy_with_zero);
   end
 end

 fprintf(file1,'%s\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\tu=%g\tn=%g\n',file,size(A,1),size(A_test,1),acc,acc_SD,min_c1,min_c2,min_a,min_e,time,min_u,n-1);
